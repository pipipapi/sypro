...
final public class DateUtil {
	 ...
	 * @param lastDate 较大日期
	 * @param formStr 格式化的日期格式
	 * @return 两个日期差的日期集合,每天为一个元素,不包含lastDate当天,例如2015-08-15~2015-08-18,那么将得到15日、16日、17日三个日期
	 * @throws ParseException 
	 */
	public static List<Date> getBetweenDaysList(Date firstDate,Date lastDate,String formStr) throws ParseException{
		...
		if(difference>0){
			list.add(DateUtil.formatDateToDate(firstDate, formStr));
		}
		for(int i=0;i<difference-1;i++){
			calendar.setTime(firstDate);
			calendar.add(Calendar.DAY_OF_YEAR, 1+i);
			list.add(DateUtil.formatDateToDate(calendar.getTime(), formStr));
		}
		return list;
	}
